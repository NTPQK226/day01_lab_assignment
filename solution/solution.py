...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_cost = (gpt4o_estimated_tokens / 1000) * COST_PER_1K_OUTPUT_TOKENS["gpt-4o"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for res in results:
        p_trunc = truncate(res.get('prompt', ''))
        g_resp = truncate(res.get('gpt4o_response', ''))
        m_resp = truncate(res.get('mini_response', ''))
        g_lat = f"{res.get('gpt4o_latency', 0):.2f}"
        m_lat = f"{res.get('mini_latency', 0):.2f}"
        
        row = f"{p_trunc:<40} | {g_resp:<40} | {m_resp:<40} | {g_lat:<15} | {m_lat:<15}"
        rows.append(row)
